...
class HistoryUTAE(nn.Module):
    ...
    def forward(self,fine,coarse,support,context,emissivity,history):
        ...
        past=self.historical(hist.reshape(b*9,9,h,w)).reshape(b,9,32,h,w)
        ...
        for i,up in enumerate(self.core.up_blocks):
            ...
            a=F.interpolate(att.reshape(heads*b,10,*att.shape[-2:]),size=shape,mode='bilinear',align_corners=False).reshape(heads,b,10,*shape)
