...
| [Download guide](docs/release.md) | Google Drive data (link pending), GitHub model weights, checksums and extraction |
...
Download the test data from Google Drive (link pending) and the selected model weights from GitHub Releases, then restore `historylst246-test` and `selected-checkpoints` as described in the [download guide](docs/release.md). From the repository root:
...
The complete processed dataset has 27 arrays totaling 10.49 GB uncompressed. Its five lossless parts (about 3.63 GB) are prepared for Google Drive; the sharing link will be added when available. Dataset arrays are not uploaded to GitHub. Selected model weights are supplied separately in GitHub Releases. All original `.npy` and checkpoint bytes are preserved. Large binary files are excluded from Git history. See [release scope](docs/release.md#scope-and-attribution) for source attribution, licenses and historical runner assumptions.
