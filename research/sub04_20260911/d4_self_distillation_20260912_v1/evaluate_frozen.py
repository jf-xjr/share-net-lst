...
PROTOCOL='d4_self_distillation_six_selected_val_weights_v1'
...
def reader():
    path=HERE/'summarize_matched.py'
    spec=importlib.util.spec_from_file_location('strict_d4_student_completion_reader',path)
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module


def source_seal():
    return {str(q.resolve()):p.sha(q) for q in (Path(__file__),HERE/'summarize_matched.py',SUB04/'evaluate_finalist.py',
