...
spec=importlib.util.spec_from_file_location('d4_benchmark_strict_freeze_reader',HERE/'summarize_matched.py')
...
def source_seal():
    return dict(benchmark.v1.source_seal(),**{str(q.resolve()):p.sha(q) for q in
        (Path(__file__),Path(benchmark.__file__),HERE/'summarize_matched.py')})
...
def run(args):
    ...
    try:
        benchmark.run(SimpleNamespace(freeze=args.freeze,output=args.output,freeze_reader='d4_self_distillation_six_selected_val_weights_v1'))
    finally:
