...
def run(args):
    ...
    def check_deadline():
        if time.time() >= deadline or time.perf_counter()-started >= 360.:
            raise TimeoutError('13:00 UTC hard cutoff or 360-second run budget reached')
    check_deadline()
    import hashlib
    ...
    if args.device == 'cuda':
        ...
        if active:
            raise RuntimeError('GPU occupied; this entry never interrupts another experiment')
    ...
    cfg.update(updates=1000, algorithm='AdamW_fused_cuda', optimizer_fused=(args.device=='cuda'), original_seed=args.original_seed,
               ...
               hard_deadline_unix=deadline, per_run_limit_seconds=360, ground_truth_loss_weight=.5, teacher_loss_weight=.5, teacher_cache_sha256=teacher_info['teacher']['sha256'], protocol='d4_self_distillation_20260912_v1')
    ...
    for path in (Path(__file__).resolve(), HERE / 'design.json',
                 ...
                 HERE / 'synthetic_check.py', freeze_path, score_paths[0],
...
def main():
    ...
    if args.mode == 'check':
        ...
        return
    for key in ('checkpoint', 'source_completion', 'output', 'teacher_receipt'):
