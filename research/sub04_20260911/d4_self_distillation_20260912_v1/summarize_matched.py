...
def run(args):
    ...
    for key,path in zip(ORDER,paths):
        ...
        cfg=meta['config'];source=original[key]
        expected=dict(design['recipe'],architecture=key[0],original_seed=key[1],protocol='d4_self_distillation_20260912_v1',
            algorithm='AdamW_fused_cuda',optimizer_fused=True,arm='full_control',initialization_sha256=source['checkpoint_sha256'])
        ...
        require(done['status']=='complete' and done['updates']==1000 and done['validation_weight_candidates']==10
            and done['selection_not_changed_by_fp32_gate'] and done['test_opened'] is False and 0<done['seconds']<=360,
    ...
    result.update(status='six_actual_d4_self_distillation_runs_complete',split='validation',
        ...
        teacher_cache_not_reopened_by_summarizer=True,
        matched_additional_budget='Same1000 updates/10choices per architecture/seed; all historical total search costs remain unequal',
    ...
    files[str(args.runs)]=sha(args.runs);files[str(HERE/'design.json')]=sha(HERE/'design.json');files[str(Path(__file__).resolve())]=sha(__file__)
    files[str(Path(paired.__file__).resolve())]=sha(paired.__file__)
    freeze=dict(protocol='d4_self_distillation_six_selected_val_weights_v1',frozen_at_utc=datetime.now(timezone.utc).isoformat(),
        checkpoints=entries,source_and_evidence_sha256=files,schedule=shared,validation=result,
        teacher_receipt=teacher_binding,teacher=teacher,
...
def read_freeze(path):
    ...
    frozen=read(path)
    require(frozen['protocol']=='d4_self_distillation_six_selected_val_weights_v1'
    ...
    for entry in frozen['checkpoints']:
        ...
        require(cfg['protocol']=='d4_self_distillation_20260912_v1' and cfg['updates']==1000 and cfg['optimizer_fused'] is True
            ...
            and done['teacher_cache_sha256']==frozen['teacher']['teacher']['sha256'], 'Frozen selected student mismatch')
    return frozen
