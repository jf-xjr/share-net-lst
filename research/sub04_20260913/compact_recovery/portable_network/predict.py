"""Portable single-view compact LST inference from six already encoded NPZ fields.
...
def verified_manifest():
    ...
    if (manifest['protocol']!='portable_compact_three_seed_inference_v1'
...
def model_class():
    ...
    return CompactFourHeadHistoryNAF
...
def load_model(manifest,seed,device):
    ...
    cfg=saved['config']
    if (cfg['protocol']!='compact_or_full_annealed_teacher_recovery_v1'
            or cfg['architecture']!='compact' or cfg['original_seed']!=seed
            or cfg['updates']!=6000 or cfg['teacher_used_at_inference'] is not False
