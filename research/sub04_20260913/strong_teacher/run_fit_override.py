...
Fit603-only explicit compute-allocation override after a preserved failed Val screen.
...
def run(args):
    if args.split != 'fit' or args.override_receipt is None:
        raise ValueError('Fit-only execution requires explicit --override-receipt')
    if args.output != (HERE / 'fit').resolve():
        raise ValueError('This one-cache authorization fixes the sole output path')
    ...
    try:
        ...
        if args.split == 'fit':
            ...
            override = read(args.override_receipt)
            if (validation['status'] != 'complete_fixed24_teacher_feasibility'
                or validation['teacher_feasibility_gate_pass'] is not False
                or validation['macro']['rmse'] != 0.40725759545661777
                or validation['improvement_over_prior_teacher_k'] != 0.002211273754502585
                or validation['freeze_sha256'] != p.sha(FREEZE)):
                raise ValueError('Require the preserved actual failed fixed24 compute screen')
            if (override['status'] != 'root_authorized_one_fit_cache_after_failed_compute_screen'
                or override['original_screen_gate_pass'] is not False
                or override['teacher_results'] != bind(result_path)
                or override['checkpoint_sha256'] != [e['checkpoint_sha256'] for e in entries]
                or override['freeze_sha256'] != p.sha(FREEZE)
                or override['views'] != list(range(8)) or override['equal_view_count'] != 24
                or override['teacher_or_view_reselected'] is not False
                or override['user_goal1_standard_lowered'] is not False
                or override['user_goal1_test_macro_threshold_k'] != .42
                or override['fit_labels_allowed'] is not False or override['test_allowed'] is not False
                or override['one_cache_only'] is not True or override['job_limit_seconds'] != 600
                or override['gpu_window_deadline_unix'] != deadline_epoch):
                raise ValueError('Explicit one-cache compute override receipt does not bind this execution')
            source[str(args.override_receipt.resolve())] = p.sha(args.override_receipt)
            source[str((HERE/'run.py').resolve())] = p.sha(HERE/'run.py')
            receipt_path = Path(validation['predictions_complete']['path'])
            receipt = read(receipt_path)
            if override['teacher_prediction_receipt'] != bind(receipt_path):
                raise ValueError('Override does not bind the original complete Val prediction receipt')
        ...
        receipt = dict(status='complete_fixed24_teacher_before_labels', split=args.split, teacher=teacher,
            ...
            original_teacher_screen_gate_pass=False,
            explicit_compute_override_receipt=bind(args.override_receipt),
            user_goal_success_standard_changed=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output',type=Path); parser.add_argument('--override-receipt',type=Path); args=parser.parse_args()
