"""Finite, paired-budget strong-teacher distillation; one forward at inference.
...
def train(args):
    ...
    cfg = dict(RECIPE, architecture=args.architecture, original_seed=args.original_seed,
        algorithm='AdamW_fused_cuda', protocol='strong_teacher_kd_20260913_v1')
    ...
    model = (HistoryNAFReconstructor if args.architecture == 'naf_history' else DropoutUTAE)(history_dropout=0., emissivity_dropout=0.)
    initial = torch.load(checkpoint_path, map_location='cpu', weights_only=False)
    model.load_state_dict(initial['state_dict'], strict=True); model.cuda()
    ...
    sources = {str(p): r.digest(p) for p in [Path(__file__), HERE / 'five_hour_extension.json',
    ...
    try:
        ...
        report = dict(status='complete', updates=3000, validation_weight_candidates=14, selected_step=selected['step'],
